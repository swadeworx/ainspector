...
//           OAA_RULE_EXPORT.addElement(node, "div", "", "", item + " " + type + " " + title);
           ...
           if (type === 'sc') {
             ...
             for (var i = 0; i < rules.length; i++) {
               ...
               if (rule.wcag_primary === item) {
                 var div_node = OAA_RULE_EXPORT.addElement(section_node, "li", "", "", "");              
                 OAA_RULE_EXPORT.translateMessageCodes(div_node, "* " + rule['nls_rule_id'] + ": " + rule['definition']);
       ...
       */
       ...
       OAA_RULE_EXPORT.showRuleCategorySummary = function(node) {
         ...
         for (var category in OAA_RULE_EXPORT.CATEGORIES) {
           ...
           OAA_RULE_EXPORT.addElement(section_node, "h3", "", "", "=== " + category_name + " ===");
       ...
       */
       ...
       OAA_RULE_EXPORT.showRules = function() {
         ...
         var node = document.getElementById('ID_DIV_RULE_SUMMARY');

         OAA_RULE_EXPORT.showRuleCategorySummary(node);

         OAA_RULE_EXPORT.showWCAG20Summary(node);
