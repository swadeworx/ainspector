  ...
  function(FBTrace, Locale, OpenAjax) {
    ...
    //       check if the node is off screen or hidden from assistive technologies
             if (computed_style.is_visible_onscreen === VISIBILITY.HIDDEN) {
               ...
               case SEVERITY.VIOLATION:
                 this.insertDIV(node, tag_name, Locale.$STR('styleViolations'), 1, 0, 0, 0, 0);
                 break;
    
               case SEVERITY.WARNING: 
                 this.insertDIV(node, tag_name, Locale.$STR('styleWarnings'), 0, 1, 0, 0, 0);
                 break;
    
               case SEVERITY.MANUAL_CHECK: 
                 if (this.show_page_manual_check && node_result.isScopePage()) this.insertDIV(node, tag_name, Locale.$STR('styleManualChecks'), 0, 0, 1, 0, 0);
                 if (this.show_element_manual_check && node_result.isScopeElement()) this.insertDIV(node, tag_name, Locale.$STR('styleManualChecks'), 0, 0, 1, 0, 0);
                 
                 break;
                 
               case SEVERITY.PASS: 
                 if (this.show_pass) this.insertDIV(node, tag_name, Locale.$STR('stylePassed'), 0, 0, 0, 1, 0);
                 break;
    
               default:
                 if (this.show_hidden) this.insertDIV(node, tag_name, Locale.$STR('styleHidden'), 0, 0, 0, 0, 1);
                 break;
               }
             }
           }
