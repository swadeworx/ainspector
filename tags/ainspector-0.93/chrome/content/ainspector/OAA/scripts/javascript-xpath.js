...
// TY modified to add methods/objects to a library object rather than directly
// to a window or document object.
var install = function(library) {

	library = library || this;
    var undefined = library.undefined;

    library.XPathExpression = function(expr) {
        if (!expr.length) {
    ...
    library.XPathExpression.prototype.evaluate = function(node, type) {
        return new library.XPathResult(this.expr.evaluate(new Ctx(node)), type);
    };
    
    library.XPathResult = function (value, type) {
        if (type == 0) {
            switch (typeof value) {
                case 'object':  type ++; // 4
                case 'boolean': type ++; // 3
    ...
    library.XPathResult.prototype.iterateNext = function() { return this.nodes[this.index++] };
    library.XPathResult.prototype.snapshotItem = function(i) { return this.nodes[i] };
    
    library.XPathResult.ANY_TYPE = 0;
    library.XPathResult.NUMBER_TYPE = 1;
    library.XPathResult.STRING_TYPE = 2;
    library.XPathResult.BOOLEAN_TYPE = 3;
    library.XPathResult.UNORDERED_NODE_ITERATOR_TYPE = 4;
    library.XPathResult.ORDERED_NODE_ITERATOR_TYPE = 5;
    library.XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE = 6;
    library.XPathResult.ORDERED_NODE_SNAPSHOT_TYPE = 7;
    library.XPathResult.ANY_UNORDERED_NODE_TYPE = 8;
    library.XPathResult.FIRST_ORDERED_NODE_TYPE = 9;
    
    
    library.createExpression = function(expr) {
        return new library.XPathExpression(expr, null);
    };
    
    library.evaluate = function(expr, context, _, type) {
        return library.createExpression(expr, null).evaluate(context, type);
    };
};

var OpenAjax = window.OpenAjax;

// Create a browser-agnostic library object for handling xpaths
if (typeof OpenAjax.a11y.xpath == "undefined") {
	OpenAjax.a11y.xpath = {}
	OpenAjax.a11y._xpathInternal = {};
	install(OpenAjax.a11y._xpathInternal);
}

if (!config.hasNative) {
	OpenAjax.a11y.xpath.XPathResult = OpenAjax.a11y._xpathInternal.XPathResult;
	OpenAjax.a11y.xpath.evaluate = OpenAjax.a11y._xpathInternal.evaluate;
} else {
	OpenAjax.a11y.xpath.XPathResult = window.XPathResult;
	OpenAjax.a11y.xpath.evaluate = function(xpathExpression, contextNode, namespaceResolver, resultType, result) {
		var ret = null;
		var doc = contextNode.ownerDocument;
		if (!doc && contextNode.evaluate) {
			doc = contextNode;
		}
		if (doc && doc.evaluate) {
			ret = doc.evaluate(xpathExpression, contextNode, namespaceResolver, resultType, result);
		} else {
			// when all else fails, back up to the non-native version
			ret = OpenAjax.a11y._xpathInternal.evaluate(xpathExpression, contextNode, namespaceResolver, resultType, result);
		}
		return ret;
	};
}
